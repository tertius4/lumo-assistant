...
class LumoViewProvider implements vscode.WebviewViewProvider {
  ...
  private updateStatusBar() {
    ...
    this.statusBar.backgroundColor = new vscode.ThemeColor(
      pct > 95 ? "statusBarItem.warningBackground" : "statusBarItem.infoBackground",
    );
  }
    ...
    if (selected) {
      ...
      if (confirm === "Remove") {
        ...
        vscode.window.showInformationMessage(
          `Removed "${selected.meta.name}". Freed ~${Math.round(selected.meta.tokens / 1000)}k tokens.`,
        );
      }
    }
  }
  ...
  private async handleUserMessage(text: string) {
    ...
    try {
      ...
      const response = await this.apiClient.chat(this.messages, accessToken, workspaceContext, useCookieFallback);
  ...
  private async getWorkspaceContext(): Promise<any> {
    ...
    if (workspaceFolders) {
      try {
        ...
        console.warn("File count error:", e);
      }
    }
    ...
    if (activeEditor) {
      ...
      context.activeFile = {
        name: doc.fileName,
        relativePath: workspaceFolders ? path.relative(workspaceFolders[0].uri.fsPath, doc.fileName) : doc.fileName,
  ...
  private async getWebviewContent(initialMessages: ChatMessage[] = []): Promise<string> {
    ...
    const cssUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, "src", "webview", "chatPanel.css"));
    const jsUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, "src", "webview", "chatPanel.js"));
    const svgUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, "media", "lumo-icon.svg"));
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  await workspaceIndexer.build();
  const authProvider = new LumoAuthProvider(context);
  context.subscriptions.push(vscode.authentication.registerAuthenticationProvider("lumo-auth", "Lumo", authProvider));

  // Register Sidebar View Provider
  viewProvider = new LumoViewProvider(context.extensionUri, context);
  context.subscriptions.push(vscode.window.registerWebviewViewProvider("lumo.chatView", viewProvider));
    ...
    try {
      ...
      if (textContent.includes("\ufffd")) {
        vscode.window.showWarningMessage(
          `⚠️ File "${path.basename(uri.fsPath)}" contains invalid UTF-8 characters. Some text may be garbled.`,
        );
      }
